...
if colA.button("💾 Uložiť tiket (len pre mňa)", use_container_width=True):
    tid = db.create_ticket(user["id"], _build_ticket_data(shared_flag=0))
    if send_to_pyramid and pyramid_level:
        db.update_pyramid_progress(user["id"], pyramid_level, pnl or 0)
    st.session_state.pop("ai_prefill", None)
    st.success(f"Tiket #{tid} uložený.")

if colB.button("🚀 Zdieľať do Syndikátu", type="primary", use_container_width=True):
